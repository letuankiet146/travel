<?php 
	...
	if($type == "list"){
		...
		$sql = "SELECT * FROM form_order f join customer c on f.form_order_customer_id=c.customer_id join tour t on f.form_order_tour_id=t.tour_id join status st on f.form_order_isPay=st.status_id WHERE f.form_order_delete_date is null ORDER BY form_order_id DESC LIMIT " . $offset . "," . $items ."";
		$result = mysql_query($sql,$con);
		$orders = array();
