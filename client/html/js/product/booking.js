        ...
        submitHandler: function() {
            if($("#checkbox").is(':checked')){
                if($("#thanhtoan input").is(':checked')){
                    ...
                    console.log(mydata);
                    $.ajax({
                        url: 'http://103.47.194.91:8080/spr-data/checkTour',
                        type: 'POST',
                        dataType: 'json',
                    ...
                    .done(function(data) {
                         if(data == -1){
                            alert("Email đã tồn tại. Vui lòng đăng nhập để đặt tour hoặc sử dụng email mới!!!");
                            document.getElementById("email").focus();
                         }
                         if(data == -2){
                            alert("Tour này bạn đã đặt rồi!!!");
                         }
                         if(data == -3){
                            alert("Ngày khỏi hành!!!");
                         }
                         if(data == 1){
                            load_check();
        ...
        $.each(data, function(i, val) {
            ...
            if(val.idDto == id){
                ...
                check = '<div class="oder-left">'+
                        ...
                        '<div class="row-input"><em style="margin-left: 165px">Nếu quá thời gian trên mà quý khách chưa thanh toán, IUH-Travel sẽ hủy đơn đặt tour này</em></div>'+
                        '<div class="clear"></div>'+
                        '<div class="row-input">'+
                            '<div class="div-input">'+
                ...
                $(document).ready(function() {
                     $("#btn_thanhtoan").click(function(event) {
                        ...
                        .done(function(data) {
                             window.location.href = "https://www.nganluong.vn/button_payment.php?receiver=ptthao13@gmail.com&product_name=" + code + "&price=2000&return_url=http://iuh-travel.tk/index.php?page=thanh-toan&comments=" + content + "";
                        })
                        .fail(function() {
