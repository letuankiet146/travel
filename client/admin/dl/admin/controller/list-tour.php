<?php 
	...
	if($type == "count"){
		...
		$sql= "SELECT COUNT(tour_id) FROM tour WHERE tour_active = 1";
	...
	if($type == "list"){
		...
		$sql = "SELECT * FROM tour where tour_active = 1 ORDER BY tour_id DESC LIMIT " . $offset . "," . $items ."";
		$result = mysql_query($sql,$con);
		$tours = array();
	...
	if($type == "one"){
		$lastID = (int)$_GET["id"];
		$sql = "SELECT * FROM tour where tour_active = 1 AND tour_id < " . $lastID . " ORDER BY tour_id DESC LIMIT 1";
		$result = mysql_query($sql,$con);
		$tour = array();
